...
Anonymous reads of off-book rows (`book_visible=false`) — on `GET /v1/commitments/:hash`, `?since=<cursor>` recovery, and the SSE `/v1/stream/commitments` payload — return an **allow-list projection**, not the full body. Hidden bodies expose only lifecycle and identity fields and **omit** `signature`, `nonce`, `oddsTick`, `riskAmount`, `remainingRiskAmount`, `lineTicks`, `scorer`, `speculationKey`, and every EIP-712 typed-data field — enough that no anonymous taker can reconstruct a `matchCommitment` transaction against a soft-cancelled order. The two structural discriminants `redacted: true` and `payloadAvailable: false` let consumers branch on the response shape without probing for missing fields. The default list above never serves hidden rows in the first place (filter excludes them); the projection is the contract for the paths that legitimately surface lifecycle transitions for tracked rows.
