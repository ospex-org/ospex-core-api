...
- **Own-state snapshot:** `GET /v1/own-state/snapshot?cursor=<opt>` — owner-auth (`Authorization: Bearer <stream-token>`) paged snapshot of the maker's commitments + positions. Returns `{cursor, commitments[], positions[], truncated, positionsTruncated}`. The composite cursor is the resume point for `/v1/stream/own-state` or — when `truncated: true` (commitments-only) — the continuation key for the next page. `positionsTruncated` is the snapshot's DEGRADED discriminant, decoupled from `truncated`: when `true`, position visibility is PARTIAL. Since `#76` it reports a BUDGET, not a cap — the actionable read is a bounded complete enumeration and the claimed-since read pages its keyset, so `false` now means the wallet's whole position population is in the body rather than merely that it fit under 200 rows. The stream cold-start then emits `event: degraded` before `ready` and consumers (SDK / market maker) must quote-hold and treat owner-state as partial. It is not the only route to that frame — the live per-tick derivation can exhaust its own maintenance budget after `ready` and emit it too (see "Own-state stream"), so a consumer reading `positionsTruncated: false` at connect still has to handle a later `degraded`. Since `#97` a wallet simply growing past the actionable cap is NOT one of those routes: live discovery is a keyset drain with no population cap, so it delivers the new rows instead of reporting that it cannot see them. When a wallet's population exceeds those budgets the snapshot serves the capped read and says so rather than erroring — erroring would be a reconnect loop, because the SDK retries a `resync` with no backoff. The `/v1/positions/:address` REST endpoint still provides full history out-of-band for operator tooling. **Passive expiry** (a commitment whose only terminal transition is `expiry <= now`) is NOT emitted by recovery — the indexer doesn't advance `row_updated_at` for time alone, so such rows fall outside both halves of the recovery response. The SDK reducer is responsible for computing effective status locally (same `deriveEffectiveStatus` pattern used by `/v1/commitments`) and pruning any locally-held active commitment whose stored expiry has lapsed; the active set the snapshot returns is authoritative of currently-matchable rows.
- **Own-state stream:** `GET /v1/stream/own-state` — owner-auth composite SSE stream of the caller's own commitments + position-status transitions. Cold start (no cursor) emits an inline `snapshot` event; reconnects resume from the composite cursor via `Last-Event-ID` or `?cursor=`. Emits `degraded`, **at most once per connection**, when owner-state visibility is partial: before `ready` when the cold-start snapshot or the resume catch-up is already short, or mid-stream when the live derivation saturates its per-tick bound. The frame is not repeated even when more than one of those notices, and a mid-stream one is durable for the SDK (only a fresh `ready` clears its status latch). They are not all the same condition: the cold-start and catch-up legs report this connection's own seed read of the actionable set at its 200-row cap, while the live derivation reports exhausting its per-tick maintenance budget — 400 tracked keys that can still transition — which the snapshot's cap says nothing about. The live derivation's DISCOVERY half cannot report anything: it drains `(row_updated_at, id)` ascending from its own tip, so it has no population cap to saturate.
...
Every list row — both the default forward window and `?date=` — carries the linked game's canonical identity as two deliberately redundant keys, always present: **`gameId`** and **`jsonoddsId`**, both serving the contest's `jsonodds_id`. `gameId` uses the games-surface name — `/v1/games` serves the *same string* as its `gameId`, since the games table's primary key is `(network, jsonodds_id)` and there is no separate surrogate game UUID — giving consumers an exact-equality join key between contest rows and the games surface, so that team-name + start-time matching (the served start times are derived bounds, not equality-comparable across endpoints — see the name-collision note below) demotes to defense-in-depth. `jsonoddsId` is the same value under the naming the detail endpoint has always used; the pair mirrors the intentional `gameId` / `externalIds.jsonodds` redundancy on `/v1/games`. The value is the contest row's own linkage, deliberately not gated on a `games` row existing — identity stays servable even when the games mirror row is absent (dated `gameFinalType` is then `""`). The detail, `?since=` recovery, and SSE stream bodies are unchanged (detail already carries `jsonoddsId`).

Both keys serve `null` when the contest has no JSONOdds linkage. **`null` means *no identity*, not an identity whose value is null: treat the row as non-joinable and reject or skip it before any identity comparison.** Only non-null ids participate in equality joins — comparing the nulls themselves would still match (`null === null` is `true` in JavaScript). An empty-string linkage is normalized to `null` so a missing identity always surfaces as that one well-known sentinel; served verbatim, `""` is value-shaped — it would slip past a `=== null` missing-check and false-match another unlinked row.

#### Dated discovery (`?date=YYYY-MM-DD`)

`date=` replaces the forward-only hours window with one UTC calendar day — `[date 00:00Z, date+1 00:00Z)`, half-open — on the same `effective_start_time` the listing orders on and serves as `matchTime`. It exists for the settle-and-claim half of an agent's lifecycle: after a day's games, "which contests from date D exist, are their events final, and are they scored yet?" is answerable signer-free from this one call. Dated results order on `effective_start_time` with a unique `contest_id` tiebreaker — a whole-day enumeration must paginate stably across tied start instants, and slates cluster on shared starts; the forward listing keeps its single-key ordering. Eligibility (unverified contests stay excluded), the `sport` / `status` filters (`date` + `status=scored` composes), and the limit caps are the default listing's. `date` must be a real calendar date — `2026-02-30` is rejected, not normalized — inside the accepted domain `0100-01-01` through `9999-12-30`: years below 0100 are refused (JS `Date.UTC` remaps years 0–99 into 1900–1999, and the parser rejects the remap rather than silently shifting the century), and `9999-12-31` is refused because its next-day upper bound leaves the 4-digit ISO year domain. `date` is mutually exclusive with `window`.

Each dated row additionally carries **`gameFinalType`** — the linked game's `games.final_type`, verbatim: the upstream feed's result status as free text (e.g. `Finished`, `Postponed`, `Canceled` — upstream spellings), `""` when no games row is linked or the feed has reported no result status. Event completion is signaled by `gameFinalType === 'Finished'` — not by the games table's inert `status` column, which this endpoint deliberately does not serve. The contest's own scored state is already on the row as `status` (`scored` / `scored_manually`) — postgame needs both signals, and a dated row carries both. The default (no-`date`) response does **not** carry `gameFinalType`: no row gains the key, no `games` query runs, and the query keeps its own window, filters, and ordering — each of those pinned by tests. (Both modes share one select string, which includes `jsonodds_id` for the identity keys above.)
...
Single contest detail. Returns the contest base fields a list item carries — `contestId`, teams, sport, the six start-time fields, `status`, `speculations` — plus the detail-only fields below and an `orderbook` array on each speculation populated with currently fillable commitments. Two list-row keys deliberately do not appear here: the list-only `gameId` alias (identity is served on this body as `jsonoddsId`; pinned by a scope test) and the dated-list-only `gameFinalType`. Same default filter as `GET /v1/commitments` (status `open` or `partially_filled`, not invalidated, not expired); each entry has the same wire shape as a commitment from `GET /v1/commitments`. Sorted by `createdAt` ascending; price-aware sorting is a follow-up. The list endpoint `GET /v1/contests` does not populate orderbooks.

Detail-only fields surfaced here (omitted on list rows): `rundownId`, `sportspageId`, `contestCreator`, `leagueId`, `awayScore`, `homeScore`, `contestCreatedAt`, `verifiedAt`, `scoredAt`, `voidedAt`, plus `awayTeamId` / `homeTeamId` (UUIDs from the `teams` table, resolved via `contests.network + jsonodds_id → games.{home_team_id, away_team_id}`; null when no game linkage exists). `jsonoddsId` is served here too and, since the game-identity change, on every list row (list rows additionally alias it as `gameId`; the detail body does not carry `gameId`). The team UUIDs are consumed by `@ospex/sdk`'s resolver layer to scope alias matching to a contest's two teams.
...
- **`settlementCandidates`** — all controlled positions with open speculations whose contest is `scored`, `voided`, **or** `verified` with the void cooldown elapsed (`#79`), including predicted losers and unavailable predictions. This is settlement work, **not** a payout bucket; deduplicate `speculationId` before settling. It overlaps `pendingSettle` and overlaps `active` whenever prediction inputs are missing or the contest is voided. A voided contest refunds **both** sides their own risk once `settleSpeculation` assigns `void`; that refund amount is not served in any payout bucket yet — see `docs/positions-complete-enumeration.md` for the bound.
...
`axes` are the raw stored integers with `axisScale: {min: 1, max: 5}` beside them. They are NOT 0–100: passing the raw value into an 0–100 radar collapses every polygon to ~2px from the centre, and inferring the ceiling from observed values gives softness (live max 4) a different scale from the other four. Each of the five ratings carries its own value or its own `null`, never a zero — the same never-fabricated contract stated under the detail endpoint below, from the same shared projection (`#86`). `primaryAxis` is nullable too. `matchTime` is the same bounded minimum `/v1/games` serves, not the raw column. `priceAmerican` is converted once, server-side, matching every other odds field in this service.
...
**A `date` / `slateDate` must be a real calendar date**, not merely `YYYY-MM-DD`-shaped. `2026-02-30`, `2026-02-29`, `2026-04-31`, `2026-13-01` and `0000-01-01` are each a `400 INVALID_PARAM` before any read, on this endpoint and on `/benchmark/picks` and `/benchmark/standings` alike — they share one validator. Previously all three tested the shape only, so an impossible date reached Postgres, came back `22008`, and was served as a `500 INTERNAL_ERROR`; with the publication gate unset it was worse, answering `200` with an empty collection, which reads as "nothing published" rather than as a client error. A real but implausible date (`1970-01-01`) is NOT refused — an empty page is the true answer to it, and only an impossible date is a client bug.
...
The count is opt-in because it is the part of the request whose COST clearly grows: a `participantId` scope is bounded by days of history rather than by a constant — 1,095 rows and about 27 a day.

What is constant is the **number of reads** — one for a page, two when a count is asked for — regardless of how much history exists. That is not the same as constant database work, and this page does not claim it: the view ranks over the live-scoped set, so how an anchored read's own cost moves as the ledger grows is not established here. It was measured once, at the volumes in the table above. A second measurement at a materially larger history is what would settle it.
...
**What `market` scopes.** The CLV metric family **and** the executed record and ROI. The money is scoped by the same `summarizeExecuted` over the same priced fills, grouped additionally by the receipt's market, so a market figure is one arithmetic over a subset rather than a second arithmetic that happens to agree; the all-markets case still serves the standings object verbatim, so parity is untouched. The three markets sum to the pooled totals, and a test asserts it.

`series` and `headline` are **not** scoped and say so in their own `scope` field, because those genuinely cannot be: a series point is a cohort-day's figure across markets and the headline is the basis over the whole sample. One quantity appears only in the pooled view — `unresolvedFills`, receipts the identity chain could not bind to a priced fill, which therefore have no market; a market response carries `unattributedFills` so the market totals not summing to the receipt count is legible. A market the arm never picked answers a split of **zeroes**, not an absence: `eligible` can be non-zero beside `picks: 0`, which says it was offered the market and declined it. Opportunities and picks are different denominators and a null would throw the distinction away.

**`roi` ships its own numerator and denominator.** `netUsdc`, `riskUsdc`, `pendingRiskUsdc` and `pct`, where `pct` is computed from exactly the two numbers served beside it, so the division is checkable. The denominator is `riskUsdc` **alone**: `executed.ts` accumulates staked risk on every fill unconditionally and pending risk only on undecided ones, so pending is a *subset* of staked and `staked + pending` would count it twice.

The numerator is fills with a **decided** verdict — chain-settled **or** score-predicted. It is not "settled fills": `netUsdc` accumulates whenever a payout is known, and a payout is known for a predicted verdict too, so a fill can contribute net before the chain has settled it. `roi.basis` says so and `executed.verdictSource` ships the `settled`/`predicted`/`undecided` split beside the figures. The ratio therefore mixes two populations by design — decided net over total risk — which is what #72 specifies, and it reads understated while anything is undecided. `pct` is `null`, never `0`, when nothing is at risk: an undefined ratio and a break-even return are different claims.
...
Out of scope for this batch: best-effort merge with on-chain `contests` (so each game can flag whether it has a contest). Callers can cross-check by team-name against `GET /v1/contests` (or, for free-form input, use `GET /v1/teams/aliases` plus a contest's `awayTeamId` / `homeTeamId`). Exact-id joining via a contest's `gameId` does **not** apply to this endpoint: its `gameId` comes from a different table and provider (`current_schedules.game_id`), with no proven mapping to the `games.jsonodds_id` namespace that contest `gameId` and `/v1/games`'s `gameId` share — that join is scoped to `/v1/games` ↔ `/v1/contests` (see **Game identity on list rows** above).
...
```jsonc
{
  ...
  "ownState": {
    ...
    "positionSaturationTotal": 0                       // cumulative wallet pollers whose live positionStatus derivation could not cover the population (one bump per poller, not per tick)
  },
  "connections": {
...
- `SPECULATION_MODULE_ADDRESS` — optional; the deployed `SpeculationModule`. The only thing read from it is the `uint32 public immutable i_voidCooldown` getter, once per process and cached, so that an open speculation on a `verified` contest past its cooldown can be reported as settlement work (`#79`). Reading the term from the module that would execute the settlement is what stops the two disagreeing, which a hard-coded constant or a seconds-valued env var cannot. Absent ⇒ that prediction is refused entirely and the response says so via `voidCooldownSeconds: null`
