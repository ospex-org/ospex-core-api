...
Out of scope for this batch: best-effort merge with on-chain `contests` (so each game can flag whether it has a contest). Callers can cross-check against `GET /v1/contests` by exact id — every contest list row carries `gameId`, the same string this endpoint serves as its `gameId` (see **Game identity on list rows** above) — with team-name matching (or, for free-form input, `GET /v1/teams/aliases` plus a contest's `awayTeamId` / `homeTeamId`) demoted to a fallback.
