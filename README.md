...
A consumer seeing it should read it as exactly what it is: **the retained floor is currently the minimum of the inputs.** It does **not** prove when or why the schedule moved, does not identify which source produced the lower value, and does not establish that any particular earlier start was ever observed. Do **not** infer that `gameMatchTime` is the authoritative value to gate on — `matchTime` is.
