...
Every list row — both the default forward window and `?date=` — carries the linked game's canonical identity as two deliberately redundant keys, always present: **`gameId`** and **`jsonoddsId`**, both serving the contest's `jsonodds_id` (`null` when the contest was created without a JSONOdds linkage). `gameId` uses the games-surface name — `/v1/games` serves the *same string* as its `gameId`, since the games table's primary key is `(network, jsonodds_id)` and there is no separate surrogate game UUID — giving consumers an exact-equality join key between contest rows and the games surface, so that team-name + start-time matching (which this README repeatedly warns is not equality-comparable across endpoints) demotes to defense-in-depth. `jsonoddsId` is the same value under the naming the detail endpoint has always used; the pair mirrors the intentional `gameId` / `externalIds.jsonodds` redundancy on `/v1/games`. The value is the contest row's own linkage, deliberately not gated on a `games` row existing — identity stays servable even when the games mirror row is absent (dated `gameFinalType` is then `""`). The detail, `?since=` recovery, and SSE stream bodies are unchanged (detail already carries `jsonoddsId`).
