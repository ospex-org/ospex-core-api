...
- `GET /v1/contests`, `GET /v1/contests/:contestId` — contest list / detail (renamed from `/v1/markets/*`). Carries four start-time fields: `matchTime` (the current conservative start-time safety bound — gate on this), `chainStartTime` (the immutable on-chain value), `gameMatchTime` (the odds-feed schedule), `gameEarliestMatchTime` (the game's retained safety floor). See "Contest start times" below
...
| `gameEarliestMatchTime` | The game's **current retained safety floor** (`games.earliest_match_time`), served **verbatim** — never clamped. `""` when no game row is linked. When it is the minimum of the three inputs, it is what is driving `matchTime`. Exactly one value is retained — it is not a history (see below). |
...
**The floor is served as `gameEarliestMatchTime`**, which makes its one visible consequence explicable from the body alone: after a feed rollback, `matchTime` can be **strictly less than both `chainStartTime` and `gameMatchTime`** — and in exactly that shape the served `gameEarliestMatchTime` equals the served `matchTime`, so the body itself says the retained floor is currently the minimum. That is correct behaviour, not a bug — the bound is deliberately refusing to follow the feed back up.
...
Against a database where that migration has not been applied, reads are still correct — every response computes `matchTime` from the view at read time — but a cursor-based subscriber can miss a game-only reschedule until an unrelated contest-row update or a cold snapshot.
