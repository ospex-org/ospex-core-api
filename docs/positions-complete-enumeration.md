...
- **Own-state 200-row cap:** terminal losses remain unclaimed with positive historical risk and no payout/transfer clearing path, so they can permanently consume the default capped query's budget and keep `positionsTruncated`/the MM health hold asserted. A durable correction must exclude terminal rows before applying the actionable own-state cap (without falsifying raw complete-enumeration evidence), and prove with more than 200 terminal losses plus actionable rows that health recovers without hiding live exposure. Raising the cap only delays the problem. This source finding does not establish whether any live maker is currently latched.
  ...
  **What `#76` also has to do, found while scoping it.** Making the snapshot complete is not sufficient on its own: the hub's phase-A rule treated a full page as saturation regardless of whether its cache was complete, so a complete snapshot would have been followed by a `degraded` frame on the first tick — permanently, since the frame latches per subscriber and the SDK has no in-connection event that clears a live-phase one. Probed before the change: 635 seeded keys produced 4 phase-B pages, 400 of 435 keys covered, and a latched signal. The foundation for it landed separately — `derivedStatuses.terminal`, a `coverage` claim on `seedStatusCache`, and a phase-A rule that asks "is there a key I have never seen" instead of "was the page full". There are also FOUR 200-row caps in this family, not two: `POSITION_QUERY_LIMIT`, `CLAIMED_PAGE_CAP`, `CATCHUP_POSITIONS_LIMIT` and `STATUS_DERIVATION_LIMIT`. The claimed-since population exceeds its cap for the same two wallets (295 and 293 claimed rows), and `positionsTruncated` carries both caps AND the decision to freeze the cursor's `p`, so the two have to become complete together or `p` advances on a partial view.
