...
- **Own-state 200-row cap:** terminal losses remain unclaimed with positive historical risk and no payout/transfer clearing path, so they can permanently consume the default capped query's budget and keep `positionsTruncated`/the MM health hold asserted. A durable correction must exclude terminal rows before applying the actionable own-state cap (without falsifying raw complete-enumeration evidence), and prove with more than 200 terminal losses plus actionable rows that health recovers without hiding live exposure. Raising the cap only delays the problem. This source finding does not establish whether any live maker is currently latched.
  ...
  **Both halves are now fixed, and the second one is `#97` (2026-09-23).** `derivedStatuses.terminal` carries `isTerminalForever` from the read that already did the join, so the seed retires finished keys instead of spending a first tick on them — the 635-key cold start issues ZERO phase-B queries. And the phase-A rule that treated a full discovery page as saturation is GONE, because the page is gone: discovery is an ASCENDING KEYSET DRAIN over `(row_updated_at, id)` from the poller's own tip, the same mechanism `scanCommitments`/`scanFills` already used in that file. A top-N window ordered by a mutable key can be displaced; a cursor ascending from a watermark cannot, because a page limit cuts the NEWEST rows and the next tick reads them. The relaxation withdrawn in `#96` is therefore not re-attempted — there is no longer a signal to relax, and the counterexample it failed (199 rows, a new key displaced by churn on 200 known keys) is now answered by DELIVERING the row.
  ...
  ⚠ **AND AN `.in(...)` LIST IS NOT A BOUND EITHER**, which is the second thing review caught: PostgREST's documented default response maximum is 1,000 rows, so a parent join over a longer id list SUCCEEDS with fewer rows — no error, no signal — and a position whose parent is missing from the answer is dropped by the orphan skip. Paging the positions and not their parents just moves the cap one join along. Four such reads existed on the positions path (the resume leg's two and the HUB's two; `positionFetch.ts`'s complete mode already chunked its own) plus two on the commitments path in `enrich.ts`, where the id list is the distinct contests across up to 5,000 commitments and the contest population grows with the season. All six are chunked — and that bounds the REQUEST, not every RESPONSE, which is a distinction worth keeping even now that both halves are closed. Five of the six are 1:1 on the id they are keyed by, so a 199-id chunk cannot answer with more than 199 rows and the chunk IS the whole bound. `enrich.ts`'s speculations read is keyed on `contest_id` and is one-to-MANY — a speculation per market and line beneath each contest — so its answer fanned out beneath a bounded input list and could still cross the 1,000-row maximum. Reproduced at one contest with 1,001 tuples: the last commitment came back `speculationId: null`.

  That one was filed as `#101` rather than fixed in `#100`, and then FIXED in `#102`: it now pages itself by a `speculation_id` keyset (999 rows per page, 64 pages, `uq_speculation (network, speculation_id)` is what makes the strict `>` complete), and exhausting that budget reports incompleteness instead of quietly resolving a tuple to nothing. ⚠ Read the severity correction on `#101` before treating this as cosmetic: a null `speculationId` makes the market maker's own mapper throw, which latches `ownStateMappingDegraded`, clears own-state health and raises the §5.1 posting hold — and because a cap-truncated null is deterministic rather than transient, the cold-restart self-heal re-latches instead of clearing it.

  Two general forms, and the second is the one this family keeps re-teaching:

  - **A bound you did not write is a bound the SERVER wrote for you, and it does not report itself.**
  - **Ask whether a join is 1:1 or 1:many before believing that chunking its input made it safe.** The fix for the 1:1 case looks like it covers the 1:many one, which is exactly why completeness prose has to name the axis it bounded.
